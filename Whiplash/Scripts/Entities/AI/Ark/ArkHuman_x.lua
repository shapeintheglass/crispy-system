...
ArkHuman_x = {
  ...
  Properties = {
    ...
    ArkCorruption = {
      bStartCorrupted = 0,
      attachmenteffects_CorruptedEffect = "4231073014674932361",
      attachmenteffects_CorruptedActivatedEffect = "4231073014674932994",
      attachmenteffects_CorruptedChargingEffect = "4231073014674932994",
      attachmenteffects_CorruptedTetherEffect = "4231073014674932949",
      attachmenteffects_CorruptedTetherActivatedEffect = "4231073014674932994",
      attachmenteffects_CorruptedTetherChargingEffect = "4231073014674932994",
    ...
    RaisePhantom = {bCanBeRaisedPhantom = true, metaTags_CopyToPhantom = ""},
...
function ArkHuman_x.GetUsable(A0_1657)
  local L1_1658
  L1_1658 = {}
  if A0_1657:IsLootable() and not A0_1657.arknpc:IsGlooed() then
    L1_1658["Hold Use"], ({}).ActionType = {}, "Carry"
    if not A0_1657.arknpc:IsRaisedCorpse() then
      ({}).ActionType = "ScriptDefined"
      L1_1658.Loot, ({}).DisplayText = {}, A0_1657.Properties.ArkInteractMessages.textDeadInteractText
    end
    return L1_1658
  elseif A0_1657.alternateDisplayText then
    ({}).ActionType = "ScriptDefined"
    L1_1658.Use, ({}).DisplayText = {}, A0_1657.alternateDisplayText
  elseif A0_1657.arknpc:HasUseDialog() then
    ({}).ActionType = "ScriptDefined"
    L1_1658.Use, ({}).DisplayText = {}, A0_1657.Properties.ArkInteractMessages.textAliveInteractText
  end
  return L1_1658
end
function ArkHuman_x.OnHoldUsed(A0_1659, A1_1660)
  BroadcastEvent(A0_1659, "OnCarry")
end
function ArkHuman_x.IsLootable(A0_1661)
  return A0_1661.arknpc:IsDead() or A0_1661.arknpc:IsStunned() or A0_1661.arknpc:IsAsleep()
end
function ArkHuman_x.GetRemoteManipulationInteraction(A0_1662)
  local L1_1663
  L1_1663 = {}
  if A0_1662.arknpc:IsDead() or A0_1662.arknpc:IsStunned() or A0_1662.arknpc:IsAsleep() then
    L1_1663.ActionType = "Carry"
  else
    L1_1663.ActionType = "ScriptDefined"
  end
  return L1_1663
end
function ArkHuman_x.OnResetCustom(A0_1664, A1_1665, A2_1666)
  if A1_1665 then
    A0_1664:ResetContainer()
    A0_1664:InitInventory()
    CryAction.ActivateExtensionForGameObject(A0_1664.id, "ArkEmotionExtension", true)
  end
  characterName = nil
  if A0_1664.Properties.ArkCharacter.character_Character ~= "" then
    characterName = ArkRoster.GetCharacterName(A0_1664.Properties.ArkCharacter.character_Character)
  end
  if characterName ~= nil then
    A0_1664:SetDisplayName(characterName)
  end
  A0_1664.PropertiesInstance.ArkCorruption.bIsCorrupted = A0_1664.Properties.ArkCorruption.bIsCorrupted
  A0_1664.bHasSpawnedLoot = false
  A0_1664.bHostileToPlayer = nil
  if A0_1664.Properties.bCanTriggerAreas == 1 then
    A0_1664:SetFlags(ENTITY_FLAG_TRIGGER_AREAS, 0)
  else
    A0_1664:SetFlags(ENTITY_FLAG_TRIGGER_AREAS, 2)
  end
end
function ArkHuman_x.OnNpcStunned(A0_1667)
  BroadcastEvent(A0_1667, "Stunned")
  if not A0_1667.bHasSpawnedLoot then
    A0_1667:SpawnLootFromTable()
    A0_1667.bHasSpawnedLoot = true
  end
end
function ArkHuman_x.OnNpcKilled(A0_1668)
  if not A0_1668.bHasSpawnedLoot then
    A0_1668:SpawnLootFromTable()
    A0_1668.bHasSpawnedLoot = true
  end
end
function ArkHuman_x.OnNpcUnconscious(A0_1669)
  if not A0_1669.bHasSpawnedLoot then
    A0_1669:SpawnLootFromTable()
    A0_1669.bHasSpawnedLoot = true
  end
  BroadcastEvent(A0_1669, "Sleep")
end
function ArkHuman_x.IsInvulnerable(A0_1670)
  local L1_1671
  L1_1671 = A0_1670.PropertiesInstance
  L1_1671 = L1_1671.bIsInvulnerable
  L1_1671 = L1_1671 == true
  return L1_1671
end
function ArkHuman_x.OnLureSignal(A0_1672, A1_1673, A2_1674, A3_1675)
  if A0_1672.arknpc:IsCorrupted() then
    A0_1672.arknpc:OnLured(A1_1673)
  end
end
function ArkHuman_x.OnFireThresholdExceeded(A0_1676)
  A0_1676.arknpc:LightOnFire()
end
function ArkHuman_x.OnFireThesholdFellBelow(A0_1677)
  A0_1677.arknpc:OnFireExpired()
end
function ArkHuman_x.OnFireAboveThresholdRefresh(A0_1678)
  A0_1678.arknpc:OnFireAboveThresholdRefresh()
end
function ArkHuman_x.OnSignalReceived(A0_1679)
  if not A0_1679.bHostileToPlayer and not A0_1679.arknpc:IsDead() and not A0_1679.arknpc:IsAsleep() then
    A0_1679.bHostileToPlayer = true
    ArkFaction:SetFactionDispositionTowardFaction(A0_1679.Properties.faction_ArkFaction, A0_1679.playerFaction, RELATIONSHIP_HOSTILE, true)
  end
end
